"""
GOLD/SILVER 15-MIN SCANNER - GRADE A SIGNALS ONLY

High-quality signals for real-time trading:
- Confidence 80%+
- R:R ratio 1.5:1+
- Risk < 1.5% per trade
- Volume confirmation

Expected: 2-3 signals per day (Gold + Silver combined)

Usage:
    python3 futures/scanners/gold_silver_15min_scanner.py

For continuous scanning (every 15 min):
    Add to cron or run with --continuous flag
...
class Signal:
    ...
    volume_ratio: float
    confidence: int  # 80-100 for Grade A
    grade: str  # Always 'A'

    reasoning: str


class GoldSilver15MinScanner:
    """Grade A signals only - no compromises"""

    # Minimum thresholds for Grade A
    MIN_CONFIDENCE = 80
    MIN_RR_RATIO = 1.5
    MAX_RISK_PCT = 1.5
    ...
    def fetch_data(self, ticker: str, bars: int = 400) -> pd.DataFrame:
        """Fetch 15-min data"""
        end = datetime.now()
    ...
    def scan_all(self) -> Dict[str, Optional[Signal]]:
        ...
        print(f"\n{'='*80}")
        print(f"GOLD/SILVER 15-MIN SCANNER - GRADE A ONLY")
        print(f"{'='*80}")
    ...
    def display_summary(self, results: Dict[str, Optional[Signal]]):
        ...
        if signals:
            ...
            for sig in signals:
                print(f"   {sig.instrument}: {sig.signal_type} @ ${sig.entry_price:.2f}")
                print(f"      Stop: ${sig.stop_loss:.2f} | Target: ${sig.target:.2f}")
                print(f"      {sig.pattern} | Conf: {sig.confidence}%")
        else:
            print(f"\n⏸️  No Grade A signals right now")
            print(f"   Market conditions not ideal")
        ...
        print(f"\n📊 Expected frequency: 2-3 signals/day (both instruments)")
        print(f"⏰ Run this scanner every 15 minutes for best results\n")


def main():
    """Run scanner"""
    scanner = GoldSilver15MinScanner()
